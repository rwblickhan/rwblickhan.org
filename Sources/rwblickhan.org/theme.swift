...
private struct RWBlickhanOrgHTMLFactory<Site: Website>: HTMLFactory {
    private func makeStandardBodyClass() -> Node<HTML.BodyContext> {
        ...
        let typographyProps = "prose dark:prose-invert dark:prose-headings:text-rwb-text-dark"
        let linkTypographyProps =
            "prose-a:text-rwb-link-light dark:prose-a:text-rwb-link-dark prose-a:no-underline hover:prose-a:underline"
        ...
        .header(
            .div(
                ...
                .div(
                    ...
                    .a(
                        .class("text-white dark:text-rwb-text-dark text-4xl no-underline hover:underline"),
                        .text("rwblickhan.org"),
                        .href("/index.html")),
                    .label(
                        ...
                        .for("menu-toggle"),
                        .forEach(0..<3, { _ in
                                .div(.class("h-1 w-8 bg-white dark: bg-rwb-text-dark rounded"))
                        })
                    )),
